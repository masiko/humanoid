...
int kinematics::setfp(char* fname_M, char* fname_P, char* fname_A) {
...
int kinematics::logMatrix(float angles[16]) {
	for (int i=0; i<4; i++) {
...
int kinematics::logPosition(float pos1[16]) {
...
int kinematics::logJointAngle(float ang[14]){
	for (int i=0; i<14; i++) fprintf(fp_A, "%.3f,",ang[i]);
	fprintf(fp_A, "\n");
	return 0;
}

int kinematics::readLinkFile(char fname[]) {
	FILE *fp;
...
int kinematics::readInitAngleFile(char fname[]) {
	FILE *fp;
	float angle[12];
...
int kinematics::setLegTrajectory(char *fname ,std::vector<float> &traj){
...
int kinematics::culMatrix(float m1[16], float m2[16]){
	std::cout<<"culMatrix\n";
	float m3[16];
	for (int i=0; i<4; i++) {
		for (int j=0; j<4; j++) {
...
int kinematics::setMatrixPara(unsigned int no, unsigned int frame, float* angle, float link[6]) {
...
int kinematics::setMatrix(float angle, float* link, float quat[16]) {
	float ang[3], pos[3];
...
int kinematics::getMotion(std::vector<float> &tar, unsigned int frame, float angles[14]) {
	std::cout<<"getMotion\n";
